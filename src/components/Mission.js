const Mission = () => {
    return (
        <section className="mission-area ">

            <div className="container">
                <div className="row justify-content-center">
                    <div className="col-12 col-md-8 col-lg-7">
                        <div className="intro text-center">
                            {/* <span>Our Story</span> */}
                            <h3 className="mt-3 mb-0">MISSION</h3>
                            <p>Unlike anything before the evil wizards arise from the ashes and are reborn</p>
                            <div className="row flex-lg-nowrap justify-content-center">
                                <div className="col-12 col-sm-6 text-left ">
                                    <h5 data-no="1">CREATING OUR COMMUNITY</h5>
                                </div>
                                <div className="col-12 col-sm-6 text-left">
                                    <p>We begin opening our social media and discord to the public</p>
                                </div>

                            </div>
                            <div className="row flex-lg-nowrap justify-content-center mt-3">
                                <div className="col-12 col-sm-6 text-left">
                                    <h5 data-no="2">LAUNCH</h5>
                                </div>
                                <div className="col-12 col-sm-6 text-left">
                                    <p>Our initial launch will be on 15th April 2022, once the Genesis NFTs have soldout, we will begin with our full collection</p>
                                </div>
                            </div>
                            <div className="row flex-lg-nowrap justify-content-center mt-3">
                                <div className="col-12 col-sm-6 text-left">
                                    <h5 data-no="3">FLOOR PRICES</h5>
                                </div>
                                <div className="col-12 col-sm-6 text-left">
                                    <ul>
                                        <li>Genesis will start at 0.1 Eth</li>
                                        <li>Non-Genesis NFTs: They will start at 0.05 Eth</li>
                                        <li>For every 100 minted it will go up by 0.03</li>
                                    </ul>
                                </div>
                            </div>
                            <div className="row flex-lg-nowrap justify-content-center mt-3">
                                <div className="col-12 col-sm-6 text-left">
                                    <h5 data-no="4">GIVE AWAYS</h5>
                                </div>
                                <div className="col-12 col-sm-6 text-left">
                                    <ul>
                                        <li>We have several kinds of giveaways. 1 for Genesis holders and 1 for Regular holders</li>
                                        <li>For Genesis giveaways they will have their own permission adn rooms for it.</li>
                                        <li>For Regular holders it will be public and any holder may enter for a chance to win</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

        </section>
    )
}

export default Mission